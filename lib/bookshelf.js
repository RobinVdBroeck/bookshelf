...
function Bookshelf(knex) {
	...
	function resolveModel(input) {
		...
		return (
			...
			(function () {
				throw new errors.ModelNotResolvedError(
					`The model ${input} could not be resolved from the registry.`,
				);
			})()
		);
	}
		...
		model(name, Model, staticProperties) {
			if (Model) {
				...
				if (_.isPlainObject(Model))
					Model = this.Model.extend(Model, staticProperties);
	...
	const Model = (bookshelf.Model = BookshelfModel.extend(
		{
			...
			morphTo(relationName, ...args) {
				...
				return BookshelfModel.prototype.morphTo.apply(
					this,
					[relationName, columnNames].concat(models),
				);
			},

			through(Source, ...rest) {
				return BookshelfModel.prototype.through.apply(this, [
					resolveModel(Source),
					...rest,
				]);
			},
	...
	const Collection = (bookshelf.Collection = BookshelfCollection.extend(
		{
			_builder: builderFn,
			through(Source, ...args) {
				return BookshelfCollection.prototype.through.apply(this, [
					resolveModel(Source),
					...args,
				]);
			},
