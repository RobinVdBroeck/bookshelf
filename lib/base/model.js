...
function ModelBase(attributes, options) {
	...
	if (typeof options.requireFetch === "boolean")
		this.requireFetch = options.requireFetch;
	if (options.tableName) this.tableName = options.tableName;
	if (
		typeof options.hasTimestamps === "boolean" ||
		Array.isArray(options.hasTimestamps)
	) {
...
ModelBase.prototype.serialize = function (options) {
	...
	if (options.shallow !== true) {
		...
		const pivotAttributes = _.mapKeys(
			pivot,
			(_value, key) => `${constants.PIVOT_PREFIX}${key}`,
		);
...
ModelBase.prototype.related = function (name) {
	return (
		this.relations[name] ||
		(this[name] ? (this.relations[name] = this[name]()) : void 0)
	);
};
...
ModelBase.prototype.saveMethod = function (options) {
	...
	if (options.patch) {
		if (options.method === "insert")
			throw new TypeError(
				`Cannot accept incompatible options: method=insert, patch=${options.patch}`,
			);
...
ModelBase.prototype.getTimestampKeys = function () {
	return Array.isArray(this.hasTimestamps)
		? this.hasTimestamps
		: constants.DEFAULT_TIMESTAMP_KEYS;
};
...
ModelBase.prototype.timestamp = function (options) {
	...
	if (
		updatedAtKey &&
		(isNewModel || this.hasChanged()) &&
		!this.hasChanged(updatedAtKey)
	) {
