...
function CollectionBase(models, options) {
	...
	if (!_.isFunction(this.model)) {
		throw new Error(
			"A valid `model` constructor must be defined for all collections.",
		);
	}
...
CollectionBase.prototype.set = function (models, options) {
	...
	for (i = 0, l = models.length; i < l; i++) {
		...
		if (order && !(existing && order.indexOf(existing) > -1))
 ...
 *
 */
CollectionBase.prototype.reduceThen = function (
	iterator,
	initialValue,
	context,
) {
...
CollectionBase.prototype.get = function (obj) {
	...
	return (
		this._byId[this.idKey(obj.id)] ||
		this._byId[obj.cid] ||
		this._byId[this.idKey(obj)]
	);
};
