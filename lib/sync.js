...
function supportsReturning(client = {}) {
	...
	return ["postgresql", "postgres", "pg", "oracle", "mssql"].includes(
...
_.extend(Sync.prototype, {
	...
	first: Promise.method(function (attributes) {
		...
		if (!_.isEmpty(formattedAttributes))
			query.where(this.prefixFields(formattedAttributes));
		query.limit(1);
				...
				if (relatedData)
					return Promise.try(function () {
						if (relatedData.isThrough()) {
							...
							relatedData.pivotColumns = through.parse(
								relatedData.pivotColumns,
							);
	...
	select: Promise.method(function () {
		...
		return Promise.bind(this)
			...
			.tap(() => {
				 ...
				 * @return {Promise}
				 */
				return this.syncing.triggerThen(
					"fetching",
					this.syncing,
					columns,
					options,
	...
	insert: Promise.method(function () {
		...
		return this.query.insert(
			...
			supportsReturning(this.query.client) && this.options.autoRefresh !== false
				? "*"
				: null,
		);
	}),
	...
	update: Promise.method(function (attrs) {
		const syncing = this.syncing,
			query = this.query;
		if (syncing.id != null)
			query.where(syncing.format({ [syncing.idAttribute]: syncing.id }));
		...
		if (supportsReturning(query.client) && this.options.autoRefresh !== false)
			query.returning("*");
