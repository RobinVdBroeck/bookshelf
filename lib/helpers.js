...
module.exports = {
	...
	fetchPage(options) {
		...
		const fetchOptions = _.omit(options, [
			"page",
			"pageSize",
			"limit",
			"offset",
		]);
		const countOptions = _.omit(fetchOptions, [
			"require",
			"columns",
			"withRelated",
			"lock",
		...
		const count = () => {
			...
			return counter
				.query((qb) => {
					...
					qb.countDistinct.apply(
						qb,
						groupColumns.length > 0 ? groupColumns : targetIdColumn,
					);
				})
	...
	saveConstraints: function (model, relatedData) {
		...
		if (
			...
			data[relatedData.key("foreignKey")] =
				relatedData.parentFk || model.get(relatedData.key("foreignKey"));
			if (relatedData.isMorph())
				data[relatedData.key("morphKey")] = relatedData.key("morphValue");
		}
	...
	morphCandidate: function (candidates, morphValue) {
		const Target = _.find(
			candidates,
			(candidate) => candidate[1] === morphValue,
		);
