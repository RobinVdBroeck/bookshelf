...
class EagerRelation extends EagerBase {
	...
	morphToFetch(relationName, relatedData, options) {
		...
		const typeColumn =
			columnNames[0] === undefined ? `${morphName}_type` : columnNames[0];
		const idColumn =
			columnNames[1] === undefined ? `${morphName}_id` : columnNames[1];
		...
		const TargetByType = _.mapValues(parentsByType, (_parents, type) =>
	...
	_eagerLoadHelper(response, relationName, handled, options) {
		const relatedData = handled.relatedData;
		const isEmptyHasOne =
			response.length === 0 && relatedData.type === "hasOne";
		const relatedModels = isEmptyHasOne
			? []
			: this.pushModels(relationName, handled, response, options);
		...
		return Promise.try(() => {
			...
			if (response.length > 0 && options.withRelated) {
				...
				return new EagerRelation(relatedModels, response, relatedModel)
					.fetch(options)
					.return(response);
			}
		}).tap(() => {
			return Promise.map(relatedModels, (model) =>
				model.triggerThen("fetched", model, model.attributes, options),
			);
		});
	}
