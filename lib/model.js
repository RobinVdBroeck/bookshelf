...
const BookshelfModel = ModelBase.extend(
	{
		...
		morphTo(morphName) {
			if (!_.isString(morphName))
				throw new Error("The `morphTo` name must be specified.");
			let columnNames, candidates;
			if (
				arguments[1] == null ||
				(Array.isArray(arguments[1]) && _.isString(arguments[1][0]))
			) {
     ...
     */
		...
		refresh(options = {}) {
			...
			if (this.isNew()) {
				...
				attributes[this.idAttribute] =
					...
					this.attributes[this.parsedIdAttribute()];
			}
			...
			return (
				this.sync(options)
					...
					.catch(this.constructor.NotFoundError, function (err) {
						if (
							(this.requireFetch && options.require !== false) ||
							options.require
						)
							throw err;
						return null;
		...
		load: Promise.method(function (relations, options) {
			...
			return this._handleEager(
				[columns],
				_.assignIn({}, options, { shallow: true, withRelated }),
			).return(this);
		}),
			...
			return Promise.bind(this)
				...
				.then(function (method) {
					...
					const getAttributesToSave = function (method, options, model) {
						return method === "update" && options.patch
							? attrs
							: model.attributes;
					};
					...
					return this.triggerThen(
						...
						.then(function () {
							const eventsToTrigger =
								method === "insert" ? "created saved" : "updated saved";
							this._reset();
		...
		_morphOneOrMany(Target, morphName, columnNames, morphValue, type) {
			...
			if (!morphName || !Target)
				throw new Error("The polymorphic `name` and `Target` are required.");
