	...
	/** @lends Relation.prototype */ {
		...
		init(parent) {
			...
			if (this.type === "morphTo" && !parent._isEager) {
				...
				this.target = Helpers.morphCandidate(
					this.candidates,
					this.parentAttributes[this.key("morphKey")],
				);
		...
		through(source, Target, options) {
			...
			if (
				...
				throw new Error(
					"`through` is only chainable from `hasOne`, `belongsTo`, `hasMany`, or `belongsToMany`",
				);
			}
		...
		key(keyName) {
			...
			switch (keyName) {
				...
				case "throughForeignKey":
					this[keyName] =
						singularMemo(this.joinTable()) + "_" + this.throughIdAttribute;
					break;
				case "foreignKey":
					switch (this.type) {
						case "morphTo": {
							const idKeyName =
								this.columnNames && this.columnNames[1]
									? this.columnNames[1]
									: this.morphName + "_id";
						...
						case "belongsTo":
							this[keyName] =
								singularMemo(this.targetTableName) +
								"_" +
						...
						default:
							...
							this[keyName] =
								singularMemo(this.parentTableName) +
								"_" +
				...
				case "morphKey":
					this[keyName] =
						this.columnNames && this.columnNames[0]
							? this.columnNames[0]
							: this.morphName + "_type";
					break;
				case "morphValue":
					this[keyName] =
						this.morphValue || this.parentTableName || this.targetTableName;
					break;
			}
			return this[keyName];
		},
		...
		attribute(attribute, parent) {
			switch (attribute) {
				case "parentIdAttribute":
					if (this.isThrough()) {
						...
						if (
							...
							this.isThroughForeignKeyTargeted()
						) {
				...
				case "targetIdAttribute":
					if (this.isThrough()) {
						if (
							(this.type === "belongsToMany" || this.type === "belongsTo") &&
							this.isOtherKeyTargeted()
						) {
							return this.otherKeyTarget;
						}
				...
				case "parentFk":
					...
					if (this.isThrough()) {
						...
						if (this.type === "belongsTo") {
							return this.throughForeignKey
								? this.parentAttributes[this.parentIdAttribute]
								: this.parentId;
						}
					...
					return this.parentAttributes[
						this.isInverse() ? this.key("foreignKey") : this.parentIdAttribute
					];
			}
		},
		...
		joinTable() {
			...
			return (
				this.joinTableName ||
				[this.parentTableName, this.targetTableName].sort().join("_")
			);
		},
		...
		relatedInstance(models, options) {
			...
			if (this.isSingle()) {
				if (!(Target.prototype instanceof ModelBase)) {
					throw new Error(
						`The ${this.type} related object must be a Bookshelf.Model`,
					);
				}
		...
		eagerPair(relationName, related, parentModels, options) {
			...
			const grouped = _.groupBy(related, (m) => {
				...
				if (m.pivot) {
					if (this.isInverse() && this.isThrough()) {
						key = this.isThroughForeignKeyTargeted()
							? m.pivot.get(this.throughForeignKeyTarget)
							: m.pivot.id;
					} else {
						key = m.pivot.get(this.key("foreignKey"));
					}
				} else if (this.isInverse()) {
					key = this.isForeignKeyTargeted()
						? m.get(this.foreignKeyTarget)
						: m.id;
				} else {
			...
			_.each(parentModels, (model) => {
				let groupedKey;
				if (!this.isInverse()) {
					const parsedKey = Object.keys(
						model.parse({ [this.parentIdAttribute]: null }),
					)[0];
				...
				if (groupedKey != null) {
					const relation = (model.relations[relationName] =
						this.relatedInstance(grouped[groupedKey], options));
					if (this.type === "belongsToMany") {
			...
			related.map((model) => {
				...
				model.formatTimestamps()._previousAttributes = _.cloneDeep(
					model.attributes,
				);
				model._reset();
			});
...
const PivotHelpers = {
	 ...
	 * @return {Promise}
	 */
	_processModelPivot: Promise.method(
		function (method, item, options, data, fks) {
			const relatedData = this.relatedData,
