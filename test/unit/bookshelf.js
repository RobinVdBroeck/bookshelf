	...
	describe("Bookshelf", function () {
		...
		describe("Construction", function () {
			...
			it("should fail if passing a random object", function () {
				assert.throws(
					() => Bookshelf({ config: "something", options: ["one", "two"] }),
					/Invalid knex/,
				);
			});
		});
		...
		describe("Collection and Model registry with relations", function () {
			...
			it("can access registered models through collection methods", function () {
				assert.deepStrictEqual(
					modelWithRelations.testHasOne().relatedData.target,
					TestModel,
				);
			});
			...
			it("can access registered collections through collection methods", function () {
				assert.deepStrictEqual(
					modelWithRelations.testHasMany().relatedData.target,
					TestCollection,
				);
			});
			...
			it("throws a ModelNotResolved error for nonexistent relations", function () {
				assert.throws(() => modelWithRelations.testNotResolved(), {
					message:
						"The model NonexistentModel could not be resolved from the registry.",
				});
			});
			...
			it("can be used in morphTo() relations", function () {
				...
				try {
					...
					if (
						error.message !==
						'The target polymorphic type "undefined" is not one of the defined target types'
					) {
						throw error;
					}

					assert.strictEqual(
						morphToSpy.calledWith(
							"morphable",
							["relType", "relId"],
							TestModel,
							[TestModel, "relValue"],
		...
		describe(".collection()", function () {
			...
			it("registers a collection", function () {
				const RegisteredCollection = bookshelf.collection(
					"TestCollection",
					TestCollection,
				);
			...
			it("returns a previously registered collection if passing a string", function () {
				...
				assert.deepStrictEqual(
					bookshelf.collection("TestCollection"),
			...
			it("preserves instance properties", function () {
				bookshelf.collection("TestCollection", TestCollection);
				assert.strictEqual(
					bookshelf.collection("TestCollection").prototype.property,
					"something",
				);
			});
		...
		describe(".model()", function () {
			...
			it("preserves instance properties", function () {
				bookshelf.model("TestModel", TestModel);
				assert.strictEqual(
					bookshelf.model("TestModel").prototype.tableName,
					"records",
				);
			});
