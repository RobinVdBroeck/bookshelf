	...
	return Promise.all([
		...
		knex("info").insert([
			...
			},
			{
				meta_id: 2,
				other_description:
					"This is an info block for an eager hasOne -> through test",
			},
		]),
		...
		knex("posts").insert([
			...
			},
			{
				...
				content:
					"Lorem ipsum Reprehenderit esse esse consectetur aliquip magna.",
			},
			{
				owner_id: 3,
				blog_id: 3,
		...
		.then(function () {
			...
			return Promise.all([
				...
				knex.raw(
					"SELECT setval('backup_types_id_seq', (SELECT MAX(id) from \"backup_types\"));",
				),
			]);
		})
		.catch(function (e) {
