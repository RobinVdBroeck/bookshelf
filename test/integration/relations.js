	...
	describe("Relations", function () {
		...
		describe("Bookshelf Relations", function () {
			describe("Standard Relations - Models", function () {
				...
				it("handles hasOne (meta)", function () {
					return new Site({ id: 1 }).meta().fetch().then(checkTest(this));
				});

				it("handles belongsToMany (posts)", function () {
					return new Author({ id: 1 }).posts().fetch().then(checkTest(this));
				});
			});
				...
				it('eager loads "hasMany" relationships when children have duplicate ids', function () {
					return new Organization({ id: 2 })
						...
						.then(function (organization) {
							expect(
								organization.related("members").pluck("name"),
							).to.include.members(["Alice", "Bob"]);
						});
				});
						...
						.then(
							...
							function (err) {
								expect(err.message).to.equal(
									"undefinedRelation is not defined on the model.",
								);
							},
						);
				});
				...
				it('eager loads "belongsToMany" models correctly and parent is not undefined', function () {
					return Post.where("blog_id", 1)
						...
						.then(function (result) {
							result.models[0]
								.related("tags")
								.relatedData.parentId.should.eql(1);
						});
				});
				...
				it("attaching event get's triggered", function (done) {
					...
					admin1
						...
						.then(function () {
							site1
								.related("admins")
								.on("attaching", function (collection, modelToAttach) {
									expect(collection).to.exist;
									expect(modelToAttach.get("username")).to.eql(
										admin1.get("username"),
									);
									done();
								});
				...
				it("creating event get's triggered", function (done) {
					...
					admin1
						...
						.then(function () {
							site1
								.related("admins")
								.on("creating", function (collection, data, options) {
									expect(collection).to.exist;
			...
			describe("Custom foreignKey & otherKey", function () {
				it("works with many-to-many (user -> roles)", function () {
					return new User({ uid: 1 }).roles().fetch().tap(checkTest(this));
				});
			...
			describe("Polymorphic associations", function () {
				it("handles morphOne (photo)", function () {
					return new Author({ id: 1 }).photo().fetch().tap(checkTest(this));
				});

				it("handles morphMany (photo)", function () {
					return new Site({ id: 1 }).photos().fetch().tap(checkTest(this));
				});
				...
				it("eager loads morphTo (photos -> imageable)", function () {
					return Photo.fetchAll({ withRelated: ["imageable"] }).tap(
						checkTest(this),
					);
				});

				it("eager loads beyond the morphTo, where possible", function () {
					return Photo.fetchAll({ withRelated: ["imageable.authors"] }).tap(
						checkTest(this),
					);
				});

				it("handles morphOne with custom columnNames (thumbnail)", function () {
					return new Author({ id: 1 }).thumbnail().fetch().tap(checkTest(this));
				});

				it("handles morphMany with custom columnNames (thumbnail)", function () {
					return new Site({ id: 1 }).thumbnails().fetch().tap(checkTest(this));
				});
				...
				it("eager loads morphMany with custom columnNames (sites -> thumbnails)", function () {
					return Site.fetchAll({ withRelated: ["thumbnails"] }).tap(
						checkTest(this),
					);
				});

				it("eager loads morphTo with custom columnNames (thumbnails -> imageable)", function () {
					return Thumbnail.fetchAll({ withRelated: ["imageable"] }).tap(
						checkTest(this),
					);
				});

				it("eager loads beyond the morphTo with custom columnNames, where possible", function () {
					return Thumbnail.fetchAll({ withRelated: ["imageable.authors"] }).tap(
						checkTest(this),
					);
				});
				...
				it("throws an error if the type attribute is not one of the expected types", function () {
					...
					return Bookshelf.knex("photos")
						...
						.catch(function (error) {
							var expectedMessage =
								'The target polymorphic type "' +
								badType +
								'" is not one of the defined target types';
							expect(error.message).to.equal(expectedMessage);
						})
						.then(function () {
		...
		describe("Issue #65, custom idAttribute with eager loaded belongsTo", function () {
			it("#65 - should eager load correctly for models", function () {
				return new Hostname({ hostname: "google.com" })
					.fetch({ withRelated: "instance" })
					.tap(checkTest(this));
			});

			it("#65 - should eager load correctly for collections", function () {
				return new Bookshelf.Collection([], { model: Hostname })
					.fetch({ withRelated: "instance" })
					.tap(checkTest(this));
			});
		});
		...
		describe("Issue #97, #377 - Eager loading on parsed models", function () {
			it("correctly pairs eager-loaded models before parse()", function () {
				return Promise.all([
					...
					expect(blog.related("parsedPosts").length).to.equal(
						parsedPosts.length,
					);
				});
			});
					...
					initialize: (function (_v) {
						return function () {
							this.on(lifecycleEvent, function () {
								throw new Error(
									"`" + lifecycleEvent + "` triggered on JoinModel()",
								);
							});
						};
					})(lifecycleEvent),
		...
		describe("Binary ID relations", function () {
			it("should group relations properly with binary ID columns", function () {
				...
				const comment1 = new CriticComment({
					critic_id: critic1Id,
					comment: "c1-1",
				});
				const comment2 = new CriticComment({
					critic_id: critic1Id,
					comment: "c1-2",
				});
				const comment3 = new CriticComment({
					critic_id: critic2Id,
					comment: "c2-1",
				});
				return Promise.all([
